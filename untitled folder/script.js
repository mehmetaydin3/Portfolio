...
function initializeApp() {
  ...
  function openProject(project) {
    ...
    if (overlay && drawer) {
      ...
      console.error('Could not find overlay or drawer elements for project:', project);
    }
  }
  ...
  function removeLockIcons() {
    ...
    [pfizerCard, publicisCard, verizonCard].forEach(card => {
      if (card) {
        const lockIcon = card.querySelector('.absolute.top-4.left-4');
        if (lockIcon) {
          lockIcon.remove();
        }
      }
  ...
  function restoreLockIcons() {
    ...
    [pfizerCard, publicisCard, verizonCard].forEach(card => {
      if (card) {
        ...
        const existingLock = card.querySelector('.absolute.top-4.left-4');
        if (!existingLock) {
          // Create and add lock icon
          const lockIcon = document.createElement('div');
          lockIcon.className = 'absolute top-4 left-4 text-lg z-10';
  ...
  if (carousel && leftBtn && rightBtn) {
    ...
    function updateArrows() {
      ...
      // Show/hide left arrow using opacity
      ...
      // Show/hide right arrow using opacity
